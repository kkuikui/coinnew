<template>
    <div class="myorder">
        <div class="wrapper">
            <div class="container">
                <Tabs value="name1" @on-click="showItem" class="tabscontent">
                    <TabPane :label="$t('uc.otcorder.unpaid')" name="name1">
                        <div class="order-table">
                            <Table :no-data-text="$t('common.nodata')" stripe
                             :columns="tableColumnsOrder" :data="tableOrder" :loading="loading"></Table>
                            <div class="pagelist">
                                <div class="pageitem">
                                    <Page v-if="totalPage > 0" :total="totalNum"
                                    :current="currentPage" show-total
                    ...
                    <TabPane :label="$t('uc.otcorder.canceled')" name="name0">
                        <div class="order-table">
                            ...
                            <div class="pagelist">
                                <div class="pageitem">
                                    <Page v-if="totalPage > 0" :total="totalNum"
                                     :pageSize="pageSize" show-total :current="currentPage" @on-change="changePage" ></Page>
                                </div>
                            </div>
                        </div>
                    </TabPane>
                    <TabPane :label="$t('uc.otcorder.appealing')" name="name4">
                        <div class="order-table">
                            ...
                            <div class="pagelist">
                                <div class="pageitem">
                                    <Page v-if="totalPage > 0" :total="totalNum"
                                     :current="currentPage" show-total @on-change="changePage" :page-size="pageSize"></Page>
                                </div>
                            </div>
                        </div>
                    </TabPane>
                    <Input v-model="ordKeyword" slot="extra"
                     @on-change="handleSearch" icon="ios-search"
                      :placeholder="$t('uc.otcorder.searchtip')" class="searchtab" style="width: 250px;margin-right: 20px;"></Input>
                </Tabs>
            </div>
        </div>
    </div>
