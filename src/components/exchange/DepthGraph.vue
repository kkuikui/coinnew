    ...
    export default {
        ...
        methods: {
          initCanvas(){
            ...
            this.canvas.width = this.canvas_cover.width = this.pWidth = Math.floor(document.getElementById("depth_chart").offsetWidth);
            this.canvas.height = this.canvas_cover.height = this.pHeight = Math.floor(document.getElementById("depth_chart").offsetHeight);
          ...
          initPoints(){
            ...
            if (buy != undefined && sell != undefined) {
              ...
              this.yObj.min = 0;
              this.yObj.max = Math.max(this.values.bid.items[this.values.bid.items.length-1].amount, this.values.ask.items[this.values.ask.items.length-1].amount);
              ...
              for(var i = buyArr.length-1; i >= 0; i--) {
                ...
                this.buyPoints[this.buyPoints.length] = {x:x,y:y,amount:buyArr[i].amount,price:buyArr[i].price};
              }
              ...
              for(var i=0;i<sellArr.length;i++) {
                ...
                this.sellPoints[this.sellPoints.length] = {x:x,y:y,amount:sellArr[i].amount,price:sellArr[i].price};
              }
            }
          },
          convertTotal(){
          ...
          drawSell(){
            ...
            this.context.lineTo(this.pWidth-50, this.sellPoints[this.sellPoints.length-1].y);
          ...
          initHoverEvent(){
            var opts = this;
            this.canvas_cover.addEventListener('mousemove', function (e) {
              var arr = 0;
              if (e.offsetX <= (opts.pWidth-50)/2)
                arr = opts.getPoint(opts.buyPoints, e.offsetX);
              else
                arr = opts.getPoint(opts.sellPoints, e.offsetX);
